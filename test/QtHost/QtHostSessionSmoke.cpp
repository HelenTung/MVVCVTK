...
bool BuildInjectedInputContextTest()
{
    ...
    event.keyCode = 'K';
    event.keySym = "K";
    const auto closedInjection = injectedContext->SendInput(event);
    injectedInteractor->SetEventInformation(3, 4, 0, 0, 'R', 0, "R");
    injectedInteractor->KeyPressEvent();
    const bool isClosedInputIsolated = !closedInjection.isSucceeded
        && injectedCallbackCount == 0 && injectedLowPriorityCount == 0;
    if (!views.SetInputsEnabled(true)) return false;
    ...
    return isClosedInputIsolated && isSynchronous && isNativeDuplicateBlocked
...
bool BuildSessionInputEndpointTest()
{
    ...
    key.keyCode = 'I';
    key.keySym = "I";
    const auto beforeStart = endpoint->SendInput(key);
    const bool isStartGateClosed = !beforeStart.isSucceeded
        && feature->GetEvents().empty();
    ...
    const auto detachedTickCount = postconditionFeature->GetTickCount();
    const bool isDetachedTickSkipped = SendTimer(nativeInteractor)
        && postconditionFeature->GetTickCount() == detachedTickCount;
    ...
    const bool isStopFeatureAttached = isRebuilt && rebuiltEndpoint
        && session.AttachFeature(stopFeature) && session.Start();
...
bool BuildSceneCameraTest()
{
    ...
    // 两个 View 的异步体产物就绪后才固定相机，避免首轮数据提交重置相机。
    const auto getVolumesReady = [&endpoints]() {
        return std::all_of(endpoints.begin(), endpoints.end(),
            [](const HostRenderViewEndpoint& current) {
                auto* props = current.renderer
                    ? current.renderer->GetViewProps() : nullptr;
                ...
                while (auto* prop = props->GetNextProp()) {
                    ...
                    auto* mapper = volume
                        ...
                        : nullptr;
                    if (mapper && mapper->GetInput()) return true;
                }
                return false;
            });
    };
    const auto productDeadline = std::chrono::steady_clock::now()
        + std::chrono::seconds(5);
    while (!getVolumesReady()
        && std::chrono::steady_clock::now() < productDeadline) {
        if (!SendFrame(views, committedEpoch)) return false;
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    if (!getVolumesReady()) return false;
