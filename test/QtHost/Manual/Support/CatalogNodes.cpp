...
QJsonObject PublishedNodes(const QString& module, const QJsonObject& graph, const QString& current)
{
    ...
    auto group = Node("published-graph", "已发布数据关系", QString("%1 个节点").arg(rows.size()) + (included.size() == 256 ? " · 部分历史未展开" : ""), {}, {}, rows);
    group["defaultExpanded"] = false; return group;
}
}
QJsonArray GetCatalogNodes(const QString& module, const QJsonObject& s, const QJsonObject& input,
    const QStringList& availableActions, const QJsonObject& result, const QJsonObject& publishedGraph)
{
    ...
    if (module == "Surface" && s["hasPreview"].toBool()) children.append(Node("surface-preview:" + s["previewRequestId"].toString(),
        "等值面预览", s["isOverlayVisible"].toBool() ? "显示" : "隐藏", {"Visibility", "ClearPreview"}));
    ...
    if (module == "Wall" && s["hasResult"].toBool()) {
        ...
        const QJsonObject patch{{"result", ref}};
        const auto value = [&](const char* key) { return s[key].isDouble() ? QString::number(s[key].toDouble(), 'g', 7) : QString("无有效值"); };
        children.append(Node("wall-result:" + ref, "壁厚结果", s["isCurrent"].toBool() ? "当前结果" : "已过期",
            {"Result", "SetEvaluation", "SetDisplay", "SetActive", "SelectSample", "Clear"},
            {{"Result", patch}, {"SetActive", patch}},
            {Node("wall-valid", "有效采样", s["validCount"].toString() + " / " + s["sampleCount"].toString()),
             Node("wall-coverage", "覆盖率", QString::number(s["coverage"].toDouble() * 100., 'f', 1) + "%"),
             Node("wall-minimum", "最小壁厚", value("minimum")), Node("wall-maximum", "最大壁厚", value("maximum")),
             Node("wall-mean", "平均壁厚", value("mean"))}));
    }
    ...
    return {Node("root", GetModuleText(module), actions.isEmpty() ? "当前构建未启用" : input["available"].toBool() ? "结果目录" : "等待输入", actions, {}, children)};
}
}
