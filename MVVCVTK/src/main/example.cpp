...
算法层还提供了 vtkImageData + virtualMaskImage 的 vtkImageMask 联动入口，
当后续需要一份“原图被 mask 后的预览 image”时，可以直接复用这条 VTK 管线，
而不是在插件里再写一套逐像素拷贝逻辑。
...
- 用 cropData 里的 bounds 构造 vtkBox，或者直接接收 widget planes 这样的 vtkImplicitFunction；
- 通过 GetMaskedPolyData / GetClippedPolyData 交给 vtkTableBasedClipDataSet；
- 最后过一层 vtkGeometryFilter 得到稳定的 vtkPolyData 输出。

也就是说，polydata 后端真正关心的是“一个 VTK implicit mask/function”。
今天这个 mask 可以是 vtkBox 或 vtkPlanes，后续如果要接 CylinderMask / vtkCylinder，
...
- 3D 主模型临时裁切不再只依赖 bounds，而是直接读取 vtkBoxRepresentation 当前 6 个平面，
    再把这些世界平面转换到模型坐标后交给 vtkTableBasedClipDataSet。
...
而 A 窗口这类 3D 等值面窗口，会直接按红色 widget 当前平面裁主模型，
避免“红框在这里、真正裁切范围却偏到另一边”的视觉错位。
...
- 如果目标窗口是 3D 且主 prop 是 vtkPolyDataMapper，则读取当前 widget 6 个平面，转换到模型坐标后做一次临时 polydata clip；
- 最后输出本次 preview 的 source / backend / inside voxel / output voxel / main3D / bounds 日志。

所以这里的耦合边界非常清楚：
main 负责“O / 1 / 2 绑定”，bridge 负责“什么时候算”和“哪些窗口该刷新”，
overlay 负责 mask / outline，3D 主 prop 可用时则额外做临时 polydata clip。
