...
VirtualCrop 不再额外生成 masked image。
它只把 virtualMaskImage 交给 overlay，overlay 用 image mapper 按当前切片显示 mask，
避免为了“预览”再复制一份整幅体数据。
...
- 3D 主模型临时裁切不再只依赖 bounds，而是直接读取 vtkBoxRepresentation 当前 6 个世界平面，
