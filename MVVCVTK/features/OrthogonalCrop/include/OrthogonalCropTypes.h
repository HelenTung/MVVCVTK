...
// 多条裁切链共用的纯几何构造；namespace 表达无对象状态、无生命周期。
namespace CropGeometry {
    CropMatrixDouble16Array GetIdentityMatrix();

    // 标准裁切盒固定为 [-1, 1]^3；所有请求只携带 boxToInputModelMatrix 作为几何真源。
    CropBoundsDouble6Array GetCanonicalBounds();

    // 从 active input model 轴对齐 bounds 构造标准盒到 active input model 的矩阵。
    CropMatrixDouble16Array GetBoxMatrix(
...
struct CropDataModel {
    ...
    CropMatrixDouble16Array boxToInputModelMatrix = CropGeometry::GetIdentityMatrix();
